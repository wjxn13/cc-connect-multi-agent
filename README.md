...
- **L1 点名关卡层**：cc-connect **引擎内**（需打 `patches/0007`）—— 读消息里**第一个** `@` 记号，
  命中别人的话在**建会话之前**就丢弃，是最省的一层
...
| 路线 | 做法 | 代价 |
|---|---|---|
| **L1 点名关卡（本仓库已落地，性价比最高）** | 引擎层读消息里第一个 `@` 记号，命中别人就丢 | 要给 cc-connect 打 `patches/0007` + `0008` 并重编译；但**0 token、连 agent 进程都不起** |
...
| `patches/` | 给 cc-connect 打的引擎侧补丁（**8 个提交，分两组**：点名路由 / hook-block 静默；含应用方法与上游现状） |
...
1. **给 cc-connect 打补丁** —— `patches/0007` + `0008`（见 `patches/README.md`），打完**重新编译**
