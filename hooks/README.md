...
## 🔴 判定规则：句首连续点名段（2026-09-20 重写，**必须与 L1 保持一致**）

钩子**自己有一份点名判定**，和 cc-connect 引擎里的 L1 关卡是**两套独立实现**。
两边语义一旦分叉，就会出现「L1 放行、L2 拦掉」，用户什么也收不到。

这件事在 2026-09-20 多点名广播上线后**真实发生过**：L1 改为「句首连着写多个 @ 时，
段内每家都放行」，而钩子还用旧的行首锚定正则 `/^@(claude|...)/` ——
`@claude，@dsh，…` 对 DSH 来说行首是 `@claude`，于是被拦在模型之前
（`l2-hook.jsonl` 铁证：`block=true namedAgent=@claude`）。**开全新会话也没用，因为问题不在会话上下文。**

现行判定（`scanLeadingMentionRun`，与 `core/engine.go` 同一套分隔符白名单）：

| 句首点名段 | 判定 |
|---|---|
| 段里**有我**（任一 `self` 别名） | **放行** —— 多播消息，每家都要答 |
| 段里**只有别人** | **拦**，`namedAgent` 记段里第一个别人的规范标签 |
| 没有段（不以 `@` 开头 / 裸 `@` / 名字不认识） | 放行 —— 去留由 L1 的 `default` 决定，不归 L2 管 |

段的解析：跳前导空白 → 循环「读 `@name` → 跳分隔符」，分隔符 =
空格 / `,` `，` / `、` / `;` `；` / `+` `&`；裸 `@`（后面不是名字字符）或任何别的字符即终止整段。
所以 `@claude 帮我叫 @dsh 干活` 的段只有 `claude`（`@dsh` 在段外，属于句中提及）。

**改判定语义时，三处要一起改**：L1 引擎（`core/engine.go`）、**本钩子**、
L3 规则文字（`config.toml` 的 `append_system_prompt` + 各 agent 的 `AGENTS.md`/`CLAUDE.md`）。
漏改 L2 的症状最好认：**关卡日志显示消息被放行、agent 也有 `turn complete`，但 `silent=true` 且
`l2-hook.jsonl` 里 `block=true`**。
